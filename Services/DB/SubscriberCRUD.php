...
function getAllSubscribers() {
	...
	$output = array('status' => $status, 'result' => $result);
...
function getAllSubscribersAPI() {
			...
			while($row = $all_subscribers->fetch_assoc()) 
			{
				...
				if ($row['fields']) {
					...
					foreach ($fields_arr as $key => $value) {
						...
						if (isset($fields_arr_inner[0]) && isset($fields_arr_inner[1]))
						{
	...
	$output = array('status' => $status, 'list' => $result);
	closeConnection($conn);
	return json_encode($output);	
}
...
function getSubscriberByEmailIDOld($email) {
	...
	$output = array('status' => $status, 'subscriber_details' => $result, 'all_fields' => $fields_raw);
	closeConnection($conn);
	return json_encode($output);
}
