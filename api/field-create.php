...
} else if (strcasecmp($field->type, 'text') != 0 && strcasecmp($field->type, 'number') != 0 && strcasecmp($field->type, 'date') != 0) {
    http_response_code(503);
    $result = new \stdClass();

    $result->status = 'field_error';
    $result->message = 'Field type can be "Text","Date" or "Number" only';
} else {
